...
class Cube2Equirec(nn.Module):
    def __init__(self, equ_h, equ_w, cube_length, CUDA=False, device_id=0):
        ...
        if self.CUDA:
            self.grid.cuda(self.device_id)
            self.orientation_mask.cuda(self.device_id)
    ...
    def _ToEquirec(self, batch, mode):
        ...
        if self.CUDA:
            output = Variable(torch.zeros(1, ch, self.equ_h, self.equ_w), requires_grad=False).cuda(self.device_id)
        else:
        ...
        for ori in range(6):
            ...
            if self.CUDA:
                masked_grid = Variable(grid * mask.float().expand(-1, -1, -1, 2)).cuda(self.device_id) # 1, self.equ_h, self.equ_w, 2
            else:
            ...
            if self.CUDA:
                sampled_image_masked = sampled_image * \
                                    Variable(mask.float().view(1, 1, self.equ_h, self.equ_w).expand(1, ch, -1, -1)).cuda(self.device_id)
            else:
                sampled_image_masked = sampled_image * \
