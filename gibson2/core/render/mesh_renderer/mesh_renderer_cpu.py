...
import numpy as np
import torch
from gibson2.core.render.mesh_renderer.Cube2Equirec_Utils import Cube2Equirec
...
from gibson2.core.render.mesh_renderer.get_available_devices import get_available_devices, get_cuda_visible_devices
...
class MeshRenderer(object):
    ...
    def __init__(self, width=512, height=512, vertical_fov=90, device_idx=0, use_fisheye=False, msaa=False):
        ...
        available_devices = get_cuda_visible_devices()
        if device_idx < len(available_devices):
            ...
            device = 0
        # print('ava', available_devices, 'device_idx', device_idx, 'device', device)
        # self.device_idx = device_idx
        # self.device_minor = device
        self.device_idx = device
        self.device_minor = device_idx
        self.msaa = msaa
    ...
    def render_robot_panorama_depth(self, pano_height, pano_width, cube_dim, cuda, device_id):
        c2e = Cube2Equirec(pano_height, pano_width, cube_dim, CUDA=cuda, device_id=device_id)
        frames = []
        depths = []
        direction_str = ['back', 'down','front', 'left', 'right', 'up']
        for instance in self.instances:
            if isinstance(instance, Robot):
                camera_pos = instance.robot.eyes.get_position()
                orn = instance.robot.eyes.get_orientation()
                e_rpy_orn = np.array(p.getEulerFromQuaternion(orn)) # yaw, pitch, roll
                e_rpy_orn[1] = 0.  # agent view angle = 0.
                e_rpy_orn_list = []
                for d in direction_str:
                    if d == 'back':
                        new_e_orn = (e_rpy_orn.copy() + np.array([0., 0., np.pi]))
                    elif d == 'down':
                        new_e_orn = np.array(e_rpy_orn.copy())
                        new_e_orn[1] = np.pi/2 - 0.000001
                    elif d == 'front':
                        new_e_orn = e_rpy_orn
                    elif d == 'left':
                        new_e_orn = (e_rpy_orn.copy() + np.array([0., 0., np.pi/2]))
                    elif d == 'right':
                        new_e_orn = (e_rpy_orn.copy() + np.array([0., 0., -np.pi/2]))
                    elif d == 'up':
                        new_e_orn = np.array(e_rpy_orn.copy())
                        new_e_orn[1] = -np.pi/2 + 0.000001
                    e_rpy_orn_list.append(new_e_orn)
                new_quat_orn_list = []
                for e in e_rpy_orn_list:
                    new_quat_orn_list.append(p.getQuaternionFromEuler(e))
                depth_vaild_direction = [0, 2, 3, 4]  # [[-1, 0, -0.7], [1, 0, 0], [0, 1, 0], [0, -1, 0]]
                depth_direction_trans = torch.tensor([1, 2, 0, 3])
                view_direction_rgbs = []
                view_direction_depths = []
                for direc_id, n_orn in enumerate(new_quat_orn_list):
                    mat = quat2rotmat(xyzw2wxyz(n_orn))[:3, :3]
                    view_direction = mat.dot(np.array([1, 0, 0]))
                    self.set_camera(camera_pos, camera_pos + view_direction, [0, 0, -1]) # [0, 0, -1])
                    render_result = self.render(modes=('rgb', '3d'), hidden=[instance])
                    # view_direction_rgbs.append(render_result[0][:, :, :3]) mirror
                    view_direction_rgbs.append(torch.flip(render_result[0][:, :, :3], [1]))
                    if direc_id in depth_vaild_direction:
                        # view_direction_depths.append(render_result[1][:, :, 2:3]) mirror
                        view_direction_depths.append(torch.flip(render_result[1][:, :, 2:3], [1]))
                view_direction_rgbs_tensor = torch.stack(view_direction_rgbs, dim=0).permute(0, 3, 1, 2).float()
                panorama = c2e(view_direction_rgbs_tensor)[0].permute(1, 2, 0)  # h ,w ,c
                frames.append(panorama)
                view_direction_depths = torch.stack(view_direction_depths, dim=0)
                depths.append(view_direction_depths[depth_direction_trans])
        frames = torch.stack(frames, dim=0)
        depths = torch.stack(depths, dim=0)
        return frames, depths


    def render_panorama_with_position_oritentation(self, positions, orientations, pano_height, pano_width, cube_dim, cuda, device_id):
        # position and orientation given, and render a panorama frame
        # position is (x, y, z), orientation is euler angle
        data_num = len(positions)
        # c2e = Cube2Equirec(pano_height, pano_width, cube_dim, CUDA=cuda, device_id=device_id)
        frames = []
        hidden_instances = []
        for instance in self.instances:
            if isinstance(instance, Robot):
                ...
                eyes_height = instance.robot.eyes.get_position()[2]
                orn_quat = instance.robot.eyes.get_orientation()
        direction_str = ['front']
        for agent_id in range(data_num):
            camera_pos = positions[agent_id].copy()  # x, y, z
            camera_pos[2] = eyes_height
            e_rpy_orn = np.array(p.getEulerFromQuaternion(orn_quat))
            e_rpy_orn[1] = 0.
            e_rpy_orn[2] = orientations[0][2]
            ...
            for direc_id, n_orn in enumerate(new_quat_orn_list):
                ...
                frames.append(torch.flip(self.render(modes='rgb', hidden=hidden_instances)[0][:, :, :3], [1]))
